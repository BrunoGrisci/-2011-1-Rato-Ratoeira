...
salvamento/carregamento com menu inicial

passei game para dentro do rato e não o contrário (senão acredito que teria que trocar rato por game.rato em vários lugares, entre outros fatores).
para isso fiz as adaptacoes necessarias e adicionei alguns itens na estrutura.

falta chamar a função save nos locais corretos (pausa?), então criei adicionei uma linha de teste que salva automaticamente depois do 5o queijo do nivel 02.
...
int main (){
    ...
    RATO rato = {{33, 13}, 0,{} , 0, BRANCO, BAIXA, {1, 0, 0, 0, 0}};
...
void imprimequeijos (TIPO_QUEIJO *queijos, RATO *rat, RATOEIRA *ratoeira, int n)
{
    int i=(*rat).game.queijoatual,w=0,j,morte=0;
    int acabou = 0;
    ...
    do
    {
        if (queijos[i].visivel == 1) {
                ...
                {
                    printf ("\a");
                    (*rat).game.pontuacao += queijos[i].valor;
                    (*rat).game.nroqueijos++;
...
int mainmenu(RATO *rat) {
    ...
    char seletor = '>';
    putchxy(cx, cy, seletor);
    printcolor(cx+4, cy, BRANCOBRILHANTE, "NOVO JOGO");
    printcolor(cx+2, cy+1, BRANCOBRILHANTE, "CARREGAR JOGO");
    linha = menu(13, 14, &cx, &cy); //LIMITEINF=10, LIMITESUP=12
    switch (linha)
    {
        case 13: jogador(rat);
                 menucor(rat);
                 menuspeed(rat); break;
        case 14: carregar = 1; break;
    }
